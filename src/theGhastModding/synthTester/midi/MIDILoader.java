...
public class MIDILoader {
	...
	public MIDILoader(FileInputStream stream) throws Exception{
		...
		if(!s.equals("MThd")){
			JOptionPane.showMessageDialog(SynthTesterMain.frame, "Error loading MIDI: File identifier not MThd", "Error", JOptionPane.ERROR_MESSAGE);
			return;
		}
		indentifier = null;
		s = null;
		...
		for(int i = 0; i < trackCount; i++){
			System.out.println("loading track " + (i + 1) + " of " + trackCount);
			Track t = new Track();
