...
import theGhastModding.synthTester.main.SynthTesterMain;
...
public class Track {
	...
	public boolean loadTrack(FileInputStream stream) throws Exception {
		...
		if(!s.equals("MTrk")){
			JOptionPane.showMessageDialog(SynthTesterMain.frame, "Error loading MIDI: Track identifier not MTrk", "Error", JOptionPane.ERROR_MESSAGE);
			return false;
		}
	...
	private MIDIEvent loadEvent(ByteArrayInputStream byteStream) throws Exception {
		...
		if(meta == 0xFF){
			...
			if(type == TEXT_EVENT){
				return new TextEvent(total, new String(data));
			}
			if(type == LYRICS){
				return new LyricsEvent(total, new String(data));
			}
